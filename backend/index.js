...
app.post('/ask', requireAuth, async (req, res) => {
    try {
        const { query, camaraId, history } = req.body;
        const authHeader = req.headers.authorization;
        const supabase = getSupabaseClient(authHeader);
        let userContext = "";
        ...
        const isVereadoresListQuestion = (text) => {
            const t = stripDiacritics(text).toLowerCase();
            return /quem\s+sao\s+os\s+vereadores|quais\s+sao\s+os\s+vereadores|lista\s+de\s+vereadores|vereadores\s+da\s+camara|vereadores\s+da\s+c[âa]mara/.test(t);
        };
        const isNextSessionQuestion = (text) => {
            const t = normalizeSpaces(text).toLowerCase();
            return /quando\s+[eé]\s+a\s+pr[oó]xima\s+sess[aã]o|qual\s+[eé]\s+a\s+pr[oó]xima\s+sess[aã]o|pr[oó]xima\s+sess[aã]o/.test(t);
        };
        const isSessionsCountThisYearQuestion = (text) => {
            const t = normalizeSpaces(text).toLowerCase();
            return /quantas\s+sess[oõ]es\s+foram\s+realizadas\s+este\s+ano|quantas\s+sess[oõ]es\s+foram\s+realizadas\s+nesse\s+ano|quantas\s+sess[oõ]es\s+este\s+ano/.test(t);
        };
        const isLegalDocumentsListQuestion = (text) => {
            const t = normalizeSpaces(text).toLowerCase();
            return /quais\s+leis\s+est[aã]o\s+cadastradas|lista\s+de\s+leis\s+cadastradas|quais\s+documentos\s+legais\s+est[aã]o\s+no\s+sistema/.test(t);
        };
        const getLastSessionForCamara = async () => {
            if (!camaraId) return null;
            const todayIso = new Date().toISOString();
            const { data, error } = await supabase
                .from('sessoes')
                .select('id, titulo, data_sessao, status')
                .eq('camara_id', camaraId)
                .lte('data_sessao', todayIso)
        ...
        const isLastSessionPresenceQuestion = (text) => {
            const t = normalizeSpaces(text).toLowerCase();
            return /quem\s+estava\s+presente\s+na\s+[uú]ltima\s+sess[aã]o|lista\s+de\s+presen[cç]a\s+da\s+[uú]ltima\s+sess[aã]o|presen[cç]as\s+da\s+[uú]ltima\s+sess[aã]o|quem\s+compareceu\s+na\s+[uú]ltima\s+sess[aã]o/.test(t);
        };
        const isLastSessionAbsenceQuestion = (text) => {
            const t = normalizeSpaces(text).toLowerCase();
            return /quem\s+faltou\s+na\s+[uú]ltima\s+sess[aã]o|vereadores\s+ausentes\s+na\s+[uú]ltima\s+sess[aã]o|faltas\s+na\s+[uú]ltima\s+sess[aã]o/.test(t);
        };
        const isLastSessionVotesSummaryQuestion = (text) => {
            const t = normalizeSpaces(text).toLowerCase();
            return /o\s+que\s+foi\s+votado\s+na\s+[uú]ltima\s+sess[aã]o|quais\s+projetos\s+foram\s+votados\s+na\s+[uú]ltima\s+sess[aã]o|resumo\s+das\s+vota[cç][oõ]es\s+da\s+[uú]ltima\s+sess[aã]o|vota[cç][oõ]es\s+da\s+[uú]ltima\s+sess[aã]o/.test(t);
        };
        const extractCandidateName = (text) => {
            const raw = normalizeSpaces(text);
            if (!raw) return null;
            const m = raw.match(/(?:quem\s+é|quem\s+eh|biografia\s+do|biografia\s+da|sobre)\s+(.+?)(?:\?|$)/i);
            if (m && m[1]) return normalizeSpaces(m[1].replace(/[.,;:!?]+$/g, ''));
            const m2 = raw.match(/^quem\s+é\s+(.+)$/i);
            if (m2 && m2[1]) return normalizeSpaces(m2[1].replace(/[.,;:!?]+$/g, ''));
            return null;
        };
        const extractCandidateFromHistory = (recentMessages) => {
            const items = Array.isArray(recentMessages) ? recentMessages : [];
            for (let i = items.length - 1; i >= 0; i--) {
                const content = normalizeSpaces(items[i]?.content);
                if (!content) continue;
                const c1 = extractCandidateName(content);
                if (c1) return c1;
                const nameLike = content.match(/([A-ZÁÂÃÀÉÊÍÓÔÕÚÇ][A-Za-zÁÂÃÀÉÊÍÓÔÕÚÇáâãàéêíóôõúç]+(?:\s+[A-ZÁÂÃÀÉÊÍÓÔÕÚÇ][A-Za-zÁÂÃÀÉÊÍÓÔÕÚÇáâãàéêíóôõúç]+){1,4})/);
                if (nameLike && nameLike[1]) return normalizeSpaces(nameLike[1]);
            }
            return null;
        };
        const isBioQuestion = (text) => {
            const t = normalizeSpaces(text).toLowerCase();
            return /biografia|quem\s+é|fale\s+um\s+pouco|me\s+fale\s+um\s+pouco|sobre\s+ele|sobre\s+ela|dele|dela/.test(t);
        };
        const findVereadores = async (candidate) => {
            const cleaned = normalizeSpaces(candidate).replace(/["'`]/g, '');
            ...
            const tokensRaw = cleaned
                ...
                .filter(t => t.length >= 3)
                .slice(0, 5);
            const tokensAscii = tokensRaw.map(stripDiacritics).filter(t => t.length >= 3);
            ...
            const tryWithTokens = async (toks) => {
                let q = baseQuery();
                for (const tk of toks) q = q.ilike('nome', `%${tk}%`);
                const { data } = await q;
                return data || [];
            };

            let results = await tryWithTokens(tokensRaw);
            if (results.length > 0) return results;
            results = await tryWithTokens(tokensAscii);
            if (results.length > 0) return results;

            if (tokensRaw.length >= 2) {
                results = await tryWithTokens([tokensRaw[tokensRaw.length - 1]]);
                if (results.length > 0) return results;
            }
            if (tokensAscii.length >= 2) {
                results = await tryWithTokens([tokensAscii[tokensAscii.length - 1]]);
                if (results.length > 0) return results;
            }

            for (const tk of tokensRaw) {
                results = await tryWithTokens([tk]);
                if (results.length > 0) return results;
            }
            for (const tk of tokensAscii) {
                results = await tryWithTokens([tk]);
                if (results.length > 0) return results;
            }
            return [];
        };
        const isVereadorVoteHistoryQuestion = (text) => {
            const t = normalizeSpaces(text).toLowerCase();
            if (!/(vereador|vereadora)/.test(t)) return false;
            return /(hist[oó]rico\s+de\s+vota[cç][oõ]es\s+do\s+vereador|como\s+votou\s+o\s+vereador|como\s+votou\s+a\s+vereadora|votos\s+do\s+vereador|votos\s+da\s+vereadora|vota[cç][oõ]es\s+do\s+vereador|vota[cç][oõ]es\s+da\s+vereadora|como\s+foi\s+o\s+voto\s+do\s+vereador|como\s+foi\s+o\s+voto\s+da\s+vereadora)/.test(t);
        };
        const extractVereadorNameFromVoteQuestion = (text) => {
            const raw = normalizeSpaces(text);
            if (!raw) return null;
            const lower = raw.toLowerCase();
            let idx = lower.indexOf('vereador');
            let term = 'vereador';
            if (idx < 0) {
                idx = lower.indexOf('vereadora');
                term = 'vereadora';
            }
            if (idx < 0) return null;
            let sub = raw.substring(idx + term.length);
            sub = sub.replace(/^[^A-Za-zÁÂÃÀÉÊÍÓÔÕÚÇáâãàéêíóôõúç]+/, '');
            ...
            let cutIndex = sub.length;
            for (const ph of stopPhrases) {
                const i = lowerSub.indexOf(ph);
                if (i >= 0 && i < cutIndex) cutIndex = i;
            }
            sub = sub.substring(0, cutIndex);
            sub = sub.replace(/[.,;:!?]+$/g, '');
            sub = normalizeSpaces(sub);
            if (sub && sub.length >= 3) return sub;
            return null;
        };
        const mentionsLastSessionInText = (text) => {
            const t = normalizeSpaces(text).toLowerCase();
            return /[uú]ltima\s+sess[aã]o|sess[aã]o\s+passada|sess[aã]o\s+mais\s+recente/.test(t);
        };
        const extractProjectNameFromQuestion = (text) => {
            const raw = normalizeSpaces(text || '');
            if (!raw) return null;
            const lower = raw.toLowerCase();
            const idxProj = lower.indexOf('projeto');
            if (idxProj < 0) return null;
            let sub = raw.substring(idxProj + 'projeto'.length);
            sub = sub.replace(/^[^A-Za-zÁÂÃÀÉÊÍÓÔÕÚÇáâãàéêíóôõúç0-9]+/, '');
            const lowerSub = sub.toLowerCase();
            const stopPhrases = [
                ' na última sessão',
                ' na ultima sessão',
        ...
        const findSingleVereadorByName = async (nameCandidate) => {
            if (!camaraId) return { vereador: null, error: 'Não foi possível identificar a câmara associada à pergunta.' };
            const cleaned = normalizeSpaces(nameCandidate);
            if (!cleaned || cleaned.length < 3) {
                return { vereador: null, error: 'O nome do vereador informado é muito curto ou não pôde ser interpretado com clareza. Tente informar o nome completo ou o nome parlamentar.' };
            }
            const nameNorm = stripDiacritics(cleaned.toLowerCase());
            const { data, error } = await supabase
                .from('vereadores')
                .select('id, nome, nome_parlamentar, partido_sigla')
                .eq('camara_id', camaraId)
                .eq('ativo', true);
            if (error) {
                return { vereador: null, error: 'Erro ao buscar vereadores no banco de dados para identificar o nome informado.' };
            }
            if (!data || data.length === 0) {
                return { vereador: null, error: 'Não há vereadores ativos cadastrados para esta câmara no momento.' };
            }
            const makeNorm = (s) => stripDiacritics(normalizeSpaces(s || '').toLowerCase());
            const targetTokens = nameNorm.split(' ').filter(t => t && t.length >= 3);
            let best = null;
            let bestScore = 0;
            const scored = [];
            for (const v of data) {
                const n1 = makeNorm(v.nome);
                const n2 = makeNorm(v.nome_parlamentar);
                let s = 0;
                if (n1 === nameNorm || (n2 && n2 === nameNorm)) s += 100;
                if ((n1 && n1.includes(nameNorm)) || (n2 && n2.includes(nameNorm))) s += 50;
                for (const tk of targetTokens) {
                    if (n1 && n1.includes(tk)) s += 10;
                    if (n2 && n2.includes(tk)) s += 10;
                }
                if (s > 0) {
                    scored.push({ vereador: v, score: s });
                    if (s > bestScore) {
                        bestScore = s;
                        best = v;
                    }
            ...
            if (!best || scored.length === 0 || bestScore < 20) {
                return { vereador: null, error: 'Não consegui encontrar um vereador que corresponda claramente ao nome informado. Confira a ortografia e tente novamente com o nome completo ou o nome parlamentar.' };
            }
            const closeMatches = scored.filter(x => bestScore - x.score < 15);
            if (closeMatches.length > 1) {
                const nomes = closeMatches.map(x => x.vereador.nome_parlamentar || x.vereador.nome).filter(Boolean);
                if (nomes.length > 1) {
                    return { vereador: null, error: `Encontrei mais de um vereador que pode corresponder ao nome informado: ${nomes.join(', ')}. Tente perguntar novamente especificando o nome completo do vereador.` };
                }
            }
            return { vereador: best, error: null };
        };

        if (camaraId && isVereadoresListQuestion(query)) {
            const { data: vereadores, error: vereadoresError } = await supabase
                ...
                .order('nome');
            if (vereadoresError) {
                return res.status(500).json({ error: 'Erro ao buscar vereadores no banco de dados.' });
            }
            if (!vereadores || vereadores.length === 0) {
                return res.json({
                    answer: 'Não encontrei vereadores cadastrados para esta câmara no sistema.',
                    sources: []
                });
            }
            const linhas = vereadores.map((v) => {
                const prefs = v.preferences || {};
                const vd = prefs.vereador || {};
                const partido = vd.partido || 'não informado';
                const nascimento = vd.data_nascimento || 'não informado';
                return `- ${v.nome} (${partido}) – Nascimento: ${nascimento}`;
            });
            const answer = `Vereadores cadastrados para esta câmara, conforme o banco de dados interno:\n\n${linhas.join('\n')}\n\nOs dados acima vêm da tabela de perfis vinculada à câmara no sistema.`;
            return res.json({ answer, sources: [] });
        }

        if (camaraId && (isLastSessionPresenceQuestion(query) || isLastSessionAbsenceQuestion(query))) {
            const sessao = await getLastSessionForCamara();
            if (!sessao) {
                return res.json({
                    answer: 'Não encontrei nenhuma sessão registrada para esta câmara no sistema. Verifique se já existem sessões cadastradas.',
                    sources: []
                });
            }
            ...
            if (presError) {
                return res.status(500).json({ error: 'Erro ao buscar presenças da última sessão no banco de dados.' });
            }
            if (!presencas || presencas.length === 0) {
                return res.json({
                    answer: `A última sessão encontrada é "${sessao.titulo}" em ${sessao.data_sessao}, mas não há registros de presença associados a ela no sistema.`,
                    sources: []
                });
            }
            const presentes = [];
            ...
            for (const p of presencas) {
                const v = p.vereador || {};
                const nome = v.nome_parlamentar || v.nome || 'Nome não informado';
                const partido = v.partido?.sigla ? ` (${v.partido.sigla})` : '';
                const label = `${nome}${partido}`;
                if (p.presente) {
            ...
            const dataStr = new Date(sessao.data_sessao).toLocaleDateString('pt-BR');
            const focoFaltas = isLastSessionAbsenceQuestion(query) && !isLastSessionPresenceQuestion(query);
            const focoPresencas = isLastSessionPresenceQuestion(query) && !isLastSessionAbsenceQuestion(query);
            let answer = `Última sessão encontrada para esta câmara: "${sessao.titulo}", realizada em ${dataStr}.\n\n`;
            if (!focoFaltas) {
                if (presentes.length > 0) {
                    answer += `Vereadores presentes (${presentes.length}):\n- ${presentes.join('\n- ')}\n\n`;
                } else {
                    answer += 'Não há nenhum vereador marcado como presente nessa sessão.\n\n';
                }
            }
            if (!focoPresencas) {
                if (ausentes.length > 0) {
                    answer += `Vereadores ausentes (${ausentes.length}):\n- ${ausentes.join('\n- ')}\n\n`;
                } else {
                    answer += 'Não há nenhum vereador marcado como ausente nessa sessão.\n\n';
                }
            }
            answer += 'As informações acima vêm exclusivamente da tabela de presenças da sessão, sem incluir justificativas detalhadas.';
            return res.json({ answer, sources: [] });
        }

        if (camaraId && isLastSessionVotesSummaryQuestion(query)) {
            const sessao = await getLastSessionForCamara();
            if (!sessao) {
                return res.json({
                    answer: 'Não encontrei nenhuma sessão registrada para esta câmara no sistema. Verifique se já existem sessões cadastradas.',
            ...
            const { data: votacoes, error: votError } = await supabase
                .from('votacoes')
                .select('titulo, tipo, status, resultado, votos_favor, votos_contra, abstencoes, created_at')
                .eq('sessao_id', sessao.id)
                .order('created_at', { ascending: true });
            if (votError) {
                return res.status(500).json({ error: 'Erro ao buscar votações da última sessão no banco de dados.' });
            }
            const dataStr = new Date(sessao.data_sessao).toLocaleDateString('pt-BR');
            if (!votacoes || votacoes.length === 0) {
                const answer = `A última sessão encontrada para esta câmara é "${sessao.titulo}", realizada em ${dataStr}, mas não há nenhuma votação registrada na tabela de votações para essa sessão.`;
                return res.json({ answer, sources: [] });
            }
            const linhas = votacoes.map((v, idx) => {
                const tipo = v.tipo === 'nominal' ? 'Votação nominal' : v.tipo === 'secreta' ? 'Votação secreta' : 'Votação simples';
                const status = v.status === 'encerrada' ? (v.resultado ? `encerrada (${v.resultado})` : 'encerrada') : v.status;
                const favor = typeof v.votos_favor === 'number' ? v.votos_favor : 0;
                const contra = typeof v.votos_contra === 'number' ? v.votos_contra : 0;
                const abst = typeof v.abstencoes === 'number' ? v.abstencoes : 0;
                const placar = `Placar: ${favor} voto(s) a favor, ${contra} contra, ${abst} abstenção(ões).`;
                return `${idx + 1}. ${v.titulo} – ${tipo} – Status: ${status}. ${placar}`;
            });
            const answer = `Votações registradas na última sessão desta câmara: "${sessao.titulo}", realizada em ${dataStr}:\n\n${linhas.join('\n')}\n\nOs dados acima vêm exclusivamente da tabela de votações da sessão (sem expor votos individuais de cada vereador).`;
            return res.json({ answer, sources: [] });
        }

        if (camaraId && isVereadorVoteHistoryQuestion(query)) {
            let nameCandidate = extractVereadorNameFromVoteQuestion(query);
            if (!nameCandidate) {
                const fromDirect = extractCandidateName(query);
                if (fromDirect) nameCandidate = fromDirect;
            }
            if (!nameCandidate && Array.isArray(history) && history.length > 0) {
                const fromHistory = extractCandidateFromHistory(history);
                if (fromHistory) nameCandidate = fromHistory;
            }
            if (!nameCandidate) {
                return res.json({
                    answer: 'Você pediu informações sobre votos de um vereador, mas não consegui identificar claramente o nome na sua pergunta ou no histórico recente da conversa. Tente perguntar novamente informando o nome completo ou o nome parlamentar do vereador.',
                    sources: []
                });
            }
            const { vereador, error: vereadorError } = await findSingleVereadorByName(nameCandidate);
            if (vereadorError) {
                return res.json({
                    answer: vereadorError,
                    sources: []
                });
            }
            const vereadorNome = vereador.nome_parlamentar || vereador.nome || 'Vereador não identificado';
            const useLastSession = mentionsLastSessionInText(query);
            if (useLastSession) {
                const sessao = await getLastSessionForCamara();
                if (!sessao) {
                    return res.json({
                        answer: `Não encontrei nenhuma sessão registrada para esta câmara no sistema. Sem uma sessão registrada, não é possível listar os votos de ${vereadorNome}.`,
                        sources: []
                    });
                }
                const { data: votacoes, error: votError } = await supabase
                    .from('votacoes')
                    .select('id, titulo, tipo, status, resultado, votos_favor, votos_contra, abstencoes, created_at')
                    .eq('sessao_id', sessao.id)
                    .order('created_at', { ascending: true });
                if (votError) {
                    return res.status(500).json({ error: 'Erro ao buscar votações da última sessão no banco de dados.' });
                ...
                const votacaoIds = votacoes.map(v => v.id).filter(Boolean);
                if (votacaoIds.length === 0) {
                    const answer = `A última sessão encontrada para esta câmara é "${sessao.titulo}", realizada em ${dataStr}, mas não há nenhuma votação com identificador válido associada a ela para analisar os votos de ${vereadorNome}.`;
                    return res.json({ answer, sources: [] });
                }
                ...
                if (votosError) {
                    return res.status(500).json({ error: 'Erro ao buscar os votos do vereador na última sessão no banco de dados.' });
                }
                const votosMap = new Map();
                if (Array.isArray(votos)) {
                    for (const v of votos) {
                        if (v && v.votacao_id) {
                            votosMap.set(v.votacao_id, v.voto);
                        }
                    }
                }
                const linhas = votacoes.map((v, idx) => {
                    const tipo = v.tipo === 'nominal' ? 'votação nominal' : v.tipo === 'secreta' ? 'votação secreta' : 'votação simples';
                    if (v.tipo === 'secreta') {
                        return `${idx + 1}. ${v.titulo} – ${tipo}. Por ser votação secreta, o sistema não divulga o voto individual do vereador.`;
                    }
                    const voto = votosMap.get(v.id);
                    let votoDesc;
                    if (voto === 'favor') {
                        votoDesc = `voto de ${vereadorNome}: FAVORÁVEL`;
                    } else if (voto === 'contra') {
                        votoDesc = `voto de ${vereadorNome}: CONTRÁRIO`;
                    } else if (voto === 'abstencao') {
                        votoDesc = `voto de ${vereadorNome}: ABSTENÇÃO`;
                    } else {
                        votoDesc = `não há registro de voto de ${vereadorNome} nesta votação (pode indicar ausência, impedimento ou votação não registrada para este vereador).`;
                    }
                    const status = v.status === 'encerrada' ? (v.resultado ? `encerrada (${v.resultado})` : 'encerrada') : v.status;
                    return `${idx + 1}. ${v.titulo} – ${tipo} – Status: ${status}. ${votoDesc}`;
                });
                const answer = `Votos de ${vereadorNome} na última sessão desta câmara: "${sessao.titulo}", realizada em ${dataStr}.\n\n${linhas.join('\n')}\n\nOs dados acima vêm diretamente das tabelas de votações e votos da sessão no sistema.`;
                return res.json({ answer, sources: [] });
            } else {
                const { data: votos, error: votosError } = await supabase
                    .from('votos')
                    .select('votacao_id, voto, created_at')
                    .eq('vereador_id', vereador.id)
                    .order('created_at', { ascending: false })
                    .limit(30);
                if (votosError) {
                    return res.status(500).json({ error: 'Erro ao buscar o histórico de votos do vereador no banco de dados.' });
                }
                if (!votos || votos.length === 0) {
                    return res.json({
                        answer: `Não encontrei nenhum registro de voto para ${vereadorNome} na base de dados de votações desta câmara.`,
                        sources: []
                    });
                }
                const votacaoIds = Array.from(new Set(votos.map(v => v.votacao_id).filter(Boolean)));
                if (votacaoIds.length === 0) {
                    return res.json({
                        answer: `Foram encontrados registros de votos relacionados a ${vereadorNome}, mas nenhum está associado a uma votação válida na base. Verifique se há dados de sessões cadastrados corretamente.`,
                        sources: []
                    });
                }
                const { data: votacoes, error: votacoesError } = await supabase
                    .from('votacoes')
                    .select('id, titulo, tipo, status, resultado, sessao_id, created_at')
                    .in('id', votacaoIds);
                if (votacoesError) {
                    return res.status(500).json({ error: 'Erro ao buscar informações das votações relacionadas ao vereador no banco de dados.' });
                }
                if (!votacoes || votacoes.length === 0) {
                    return res.json({
                        answer: `Encontrei registros de votos para ${vereadorNome}, mas não consegui localizar as votações correspondentes na tabela de votações.`,
                        sources: []
                    });
                }
                const sessaoIds = Array.from(new Set(votacoes.map(v => v.sessao_id).filter(Boolean)));
                let sessoesMap = new Map();
                if (sessaoIds.length > 0) {
                    const { data: sessoes, error: sessoesError } = await supabase
                        .from('sessoes')
                        .select('id, titulo, data_sessao, camara_id')
                        .in('id', sessaoIds)
                        .eq('camara_id', camaraId);
                    if (sessoesError) {
                        return res.status(500).json({ error: 'Erro ao buscar informações das sessões relacionadas às votações no banco de dados.' });
                    }
                    if (sessoes) {
                        sessoesMap = new Map(sessoes.map(s => [s.id, s]));
                    }
                }
                const votacaoById = new Map(votacoes.map(v => [v.id, v]));
                const votosMap = new Map();
                for (const v of votos) {
                    if (v && v.votacao_id) {
                        if (!votosMap.has(v.votacao_id)) {
                            votosMap.set(v.votacao_id, v.voto);
                        }
                    }
                }
                const items = [];
                for (const v of votos) {
                    if (!v || !v.votacao_id) continue;
                    const vot = votacaoById.get(v.votacao_id);
                    if (!vot) continue;
                    items.push({ voto: v, votacao: vot });
                }
                if (items.length === 0) {
                    return res.json({
                        answer: `Encontrei registros de votos para ${vereadorNome}, mas não consegui relacioná-los corretamente com as votações e sessões correspondentes.`,
                        sources: []
                    });
                }
                const uniqueItems = [];
                const seenVotacoes = new Set();
                for (const item of items) {
                    if (seenVotacoes.has(item.votacao.id)) continue;
                    seenVotacoes.add(item.votacao.id);
                    uniqueItems.push(item);
                }
                uniqueItems.sort((a, b) => {
                    const da = a.votacao.created_at ? new Date(a.votacao.created_at).getTime() : 0;
                    const db = b.votacao.created_at ? new Date(b.votacao.created_at).getTime() : 0;
                    return db - da;
                });
                const limited = uniqueItems.slice(0, 15);
                const linhas = limited.map((item, idx) => {
                    const votacao = item.votacao;
                    const sessao = votacao.sessao_id ? sessoesMap.get(votacao.sessao_id) : null;
                    const tipo = votacao.tipo === 'nominal' ? 'votação nominal' : votacao.tipo === 'secreta' ? 'votação secreta' : 'votação simples';
                    let contextoSessao = '';
                    if (sessao) {
                        const dataStr = sessao.data_sessao ? new Date(sessao.data_sessao).toLocaleDateString('pt-BR') : null;
                        if (dataStr) {
                            contextoSessao = `Sessão: "${sessao.titulo}" em ${dataStr}. `;
                        } else {
                            contextoSessao = `Sessão: "${sessao.titulo}". `;
                        }
                    }
                    if (votacao.tipo === 'secreta') {
                        return `${idx + 1}. ${contextoSessao}Votação: ${votacao.titulo} – ${tipo}. Por ser votação secreta, o sistema não divulga o voto individual do vereador.`;
                    }
                    const voto = votosMap.get(votacao.id);
                    let votoDesc;
                    if (voto === 'favor') {
                        votoDesc = `voto de ${vereadorNome}: FAVORÁVEL`;
                    } else if (voto === 'contra') {
                    ...
                    const status = votacao.status === 'encerrada' ? (votacao.resultado ? `encerrada (${votacao.resultado})` : 'encerrada') : votacao.status;
                    return `${idx + 1}. ${contextoSessao}Votação: ${votacao.titulo} – ${tipo} – Status: ${status}. ${votoDesc}`;
                });
                const answer = `Histórico recente de votações de ${vereadorNome} nesta câmara (da mais recente para a mais antiga, limitado às últimas ${linhas.length} votações encontradas):\n\n${linhas.join('\n')}\n\nOs dados acima vêm diretamente das tabelas de votos, votações e sessões do sistema.`;
                return res.json({ answer, sources: [] });
            }
        }

        if (camaraId && isNextSessionQuestion(query)) {
            const now = new Date();
            const { data: events, error: eventsError } = await supabase
                .from('agenda_events')
                .select('title, description, event_date, event_type, status')
                .eq('camara_id', camaraId)
                .eq('status', 'pending')
                ...
                .limit(1);
            if (eventsError) {
                return res.status(500).json({ error: 'Erro ao buscar agenda no banco de dados.' });
            }
            if (!events || events.length === 0) {
                return res.json({
                    answer: 'Não há nenhuma sessão ou evento futuro com status pendente cadastrado na agenda legislativa.',
                    sources: []
                });
            }
            const e = events[0];
            const data = new Date(e.event_date);
            const dataStr = data.toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit', year: 'numeric' });
            const horaStr = data.toISOString().substring(11, 16);
            const tipo = e.event_type === 'meeting' ? 'Sessão/Reunião' : e.event_type === 'budget' ? 'Evento Orçamentário' : e.event_type === 'sanction' ? 'Prazo de Sanção' : 'Evento';
            const desc = e.description ? `\nDescrição: ${e.description}` : '';
            const answer = `Próximo evento na agenda legislativa desta câmara:\n\nTipo: ${tipo}\nTítulo: ${e.title}\nData: ${dataStr}\nHorário (aprox.): ${horaStr}${desc}\n\nOs dados acima vêm diretamente da tabela de agenda do sistema (apenas eventos com status pendente e data futura).`;
            return res.json({ answer, sources: [] });
        }

        if (camaraId && isSessionsCountThisYearQuestion(query)) {
            const now = new Date();
            ...
            if (countError) {
                return res.status(500).json({ error: 'Erro ao contar sessões no banco de dados.' });
            }
            const total = typeof count === 'number' ? count : 0;
            const answer = `Este ano, existem ${total} sessão(ões) cadastradas no sistema para esta câmara, considerando o período de ${start.getFullYear()} inteiro.\n\nO número acima vem diretamente da tabela de sessões filtrada por câmara e ano.`;
            return res.json({ answer, sources: [] });
        }

        if (camaraId && isLegalDocumentsListQuestion(query)) {
            const { data: docs, error: docsError } = await supabase
                ...
                .order('created_at', { ascending: true });
            if (docsError) {
                return res.status(500).json({ error: 'Erro ao buscar documentos legais no banco de dados.' });
            }
            if (!docs || docs.length === 0) {
                return res.json({
                    answer: 'Não há leis ou documentos legais cadastrados na base legal desta câmara no momento.',
                    sources: []
                });
            }
            const linhas = docs.map((d, idx) => {
                const data = d.created_at ? new Date(d.created_at) : null;
                const dataStr = data ? data.toLocaleDateString('pt-BR') : 'data não informada';
                return `${idx + 1}. ${d.title} (arquivo: ${d.filename || 'não informado'}, cadastrado em ${dataStr})`;
            });
            const answer = `Leis e documentos legais cadastrados na base desta câmara:\n\n${linhas.join('\n')}\n\nEssa lista foi montada diretamente a partir da tabela de documentos legais do sistema.`;
            return res.json({ answer, sources: [] });
        }
        ...
        try {
            ...
            if (user?.id) {
                ...
                const vereador = prefs?.vereador || {};
                const fields = [];
                if (p?.nome) fields.push(`Nome: ${p.nome}`);
                if (p?.cargo) fields.push(`Cargo: ${p.cargo}`);
                if (vereador?.partido) fields.push(`Partido: ${vereador.partido}`);
                if (vereador?.data_nascimento) fields.push(`Nascimento: ${vereador.data_nascimento}`);
        ...
        const stopwords = new Set([
            'que', 'com', 'para', 'uma', 'umas', 'uns', 'numa', 'num', 'nos', 'nas', 'das', 'dos', 'por',
            'isso', 'este', 'esta', 'esse', 'essa', 'aquele', 'aquela', 'como', 'onde', 'quando', 'quem',
            'qual', 'quais', 'de', 'da', 'do', 'em', 'no', 'na', 'e', 'ou', 'se', 'ao', 'aos', 'às', 'as', 'os',
            'um', 'uma'
        ]);
        const qTokens = standalone
            .toLowerCase()
            .split(/\W+/)
            .filter(t => t.length > 2 && !stopwords.has(stripDiacritics(t)));
        const lexScore = (text) => {
            const lc = (text || '').toLowerCase();
            const lcAscii = stripDiacritics(lc);
            let s = 0;
            for (const t of qTokens) {
                const base = stripDiacritics(t);
                const c1 = lc.split(t).length - 1;
                const c2 = lcAscii.split(base).length - 1;
        ...
        const parentKeys = Object.keys(byParent).slice(0, 5);
        const parents = [];
        for (const k of parentKeys) {
            const any = byParent[k][0];
            const sid = any.metadata?.session_id;
            const title = any.metadata?.title;
            const type = any.metadata?.type;
            const { data: chunks } = await supabase
                .from('session_embeddings')
                .select('content, metadata')
                .eq('session_id', sid)
                .contains('metadata', { title })
                .contains('metadata', type ? { type } : {});
            const ordered = (chunks || []).sort((a, b) => (a.metadata?.chunk_index || 0) - (b.metadata?.chunk_index || 0));
            const full = ordered.map(c => c.content).join('\n');
            parents.push({ content: full, metadata: any.metadata });
        }
        let contextText = "";
        let totalChars = 0;
        const maxContextChars = 60000;
        const sources = [];
        const ataParents = parents.filter(p => p.metadata?.type === 'ata_final');
        const otherParents = parents.filter(p => p.metadata?.type !== 'ata_final');
        for (const p of ataParents) {
            const t = `[DOCUMENTO OFICIAL: ATA FINAL - ${p.metadata.session_title}]\n${p.content}\n\n`;
        ...
        for (const p of otherParents) {
            const t = `[Transcrição: ${p.metadata.session_title} | Bloco: ${p.metadata.title}]\n${p.content}\n\n`;
            if (totalChars < maxContextChars) {
                const remaining = maxContextChars - totalChars;
                const slice = t.slice(0, remaining);
                contextText += slice;
        ...
        const relevantSids = Array.from(new Set(parents.map(p => p.metadata?.session_id).filter(Boolean)));
        for (const sid of relevantSids) {
            const anyParent = parents.find(p => p.metadata?.session_id === sid);
            const sessionTitle = anyParent?.metadata?.session_title || '';
            const { data: tchunks } = await supabase
                .from('session_embeddings')
                .select('content, metadata')
                .eq('session_id', sid)
                .eq('metadata->>type', 'transcricao_bruta');
            const ordered = (tchunks || []).sort((a, b) => (a.metadata?.chunk_index || 0) - (b.metadata?.chunk_index || 0));
            const fullTranscript = ordered.map(c => c.content).join('\n');
            if (fullTranscript && fullTranscript.length > 0) {
                const t = `[TRANSCRIÇÃO COMPLETA: ${sessionTitle}]\n${fullTranscript}\n\n`;
                if (totalChars < maxContextChars) {
                    const remaining = maxContextChars - totalChars;
                    const slice = t.slice(0, remaining);
                    contextText += slice;
                    totalChars += slice.length;
                    sources.push({ metadata: { session_title: sessionTitle, title: 'Transcrição Completa (Truncada)' } });
                }
            }
        }

        // --- INJEÇÃO DE METADADOS GARANTIDA ---
        // Recupera as últimas 3 sessões para garantir contexto básico (Presidente, Mesa, Quórum recente)
        // mesmo que a busca vetorial falhe ou o texto seja cortado.

        // [NOVO] Busca dados da câmara para contexto institucional
        let camaraContext = "";
        try {
            const { data: camaraData } = await supabase
                .from('camaras')
            ...
            if (camaraData) {
                ...
                if (camaraData.site) {
                    camaraContext += `NOTA IMPORTANTE: O site oficial da câmara é ${camaraData.site}. Se a resposta depender de informações institucionais atualizadas (como notícias, agenda futura ou contatos) que não estejam no contexto abaixo, forneça o link e sugira que o usuário consulte o site.\n`;
                }
                camaraContext += "============================\n\n";
            }
        } catch (err) {
            console.error('Erro ao buscar dados da câmara:', err);
        }

        let vereadoresContext = "";
        try {
            const wantsBio = isBioQuestion(standalone) || isBioQuestion(query);
            if (wantsBio && camaraId) {
                const candidate =
                    extractCandidateName(standalone) ||
                    extractCandidateName(query) ||
                    extractCandidateFromHistory(recent);

                if (candidate) {
                    const vereadores = await findVereadores(candidate);
                    if (vereadores.length > 0) {
                        const lines = vereadores.map(v => {
                            const prefs = v.preferences || {};
                            const vd = prefs.vereador || {};
                            const partido = vd.partido || 'não informado';
                            const nascimento = vd.data_nascimento || 'não informado';
                            const bio = vd.biografia ? String(vd.biografia).slice(0, 900) : 'não informado';
                            return `- Nome: ${v.nome}\n  Partido: ${partido}\n  Nascimento: ${nascimento}\n  Biografia: ${bio}`;
                        });
                        vereadoresContext = `=== CADASTRO DE VEREADORES (BASE INTERNA) ===\n${lines.join('\n')}\n===========================================\n\n`;
                    }
                }
            }
        } catch {}
        ...
        const { data: recentSessions } = await supabase
            .from('sessions')
            .select('title, date, blocks, final_minutes')
            .eq('camara_id', camaraId)
        ...
        if (recentSessions && recentSessions.length > 0) {
            ...
            for (const s of recentSessions) {
                ...
                metaContext += `- Sessão: ${s.title} (${s.date})\n  • Abertura: "${snippetAbertura}"\n  • Destaques Legislativos: "${snippetProjetos}"\n`;
            }
            contextText = userContext + vereadoresContext + camaraContext + metaContext + "\n\n" + contextText;
        } else {
            contextText = userContext + vereadoresContext + camaraContext + contextText;
        }
        if (!contextText) contextText = "Nenhuma informação encontrada.";
        const qLower = standalone.toLowerCase();
        const isSpeechRank = /quem\s+discursou\s+mais|qual\s+vereador\s+mais\s+discursou|orador\s+que\s+mais\s+falou/.test(qLower);
        const isProjectsThisMonth = /quais\s+projetos\s+foram\s+votados\s+(este|nesse|neste)\s+m[eê]s/.test(qLower);
        let answer = '';
        if (isSpeechRank) {
            ...
            answer = c1.choices[0].message.content;
        } else if (isProjectsThisMonth) {
            const now = new Date();
            const startOfMonth = new Date(now.getFullYear(), now.getMonth(), 1);
            const endOfMonth = new Date(now.getFullYear(), now.getMonth() + 1, 0);
            const { data: monthSessions, error: monthError } = await supabase
                .from('sessions')
                .select('id, title, date, blocks, final_minutes')
                .eq('camara_id', camaraId)
            ...
            if (!monthSessions || monthSessions.length === 0) {
                answer = 'Não há sessões registradas neste mês no sistema. Por isso não é possível listar projetos votados neste período.';
            } else {
                let projectsContext = '';
                for (const s of monthSessions) {
                    ...
                    const relevantBlocks = blocks.filter(b => {
                        ...
                        const typeMatch = t === 'votacao' || t === 'ordem_dia' || t === 'expediente';
                        const contentMatch = c.toLowerCase().includes('projeto');
                        return typeMatch && contentMatch;
                    });
                    if (relevantBlocks.length === 0 && s.final_minutes && typeof s.final_minutes === 'string') {
                        projectsContext += `Sessão: ${s.title} (${s.date})\nFonte: Ata Final\n${s.final_minutes}\n\n`;
                    } else if (relevantBlocks.length > 0) {
                        projectsContext += `Sessão: ${s.title} (${s.date})\n`;
                        for (const b of relevantBlocks) {
                            const title = (b.title || b.type || '').toString();
                            const content = (b.content || '').toString();
                            projectsContext += `Bloco: ${title}\n${content}\n\n`;
                        }
                    }
                }
                if (!projectsContext) {
                    answer = 'Existem sessões neste mês, mas não identifiquei registros claros de projetos votados nos blocos de Expediente, Ordem do Dia ou Votações.';
                } else {
                    ...
                    answer = cProj.choices[0].message.content;
                }
            }
        } else {
            const completion = await openai.chat.completions.create({
                ...
                messages: [
                    {
                        role: 'system',
                        content: `Você é um assistente legislativo oficial da Câmara Municipal.\nResponda com base exclusivamente no contexto abaixo e nas informações estruturadas fornecidas.\nSe não houver dados suficientes no contexto para responder com segurança, explique claramente essa limitação e oriente o usuário sobre como obter a informação (por exemplo, consultando o sistema interno ou o site oficial da câmara).\nNão invente leis, números de projetos, votações, datas ou resultados que não estejam presentes no contexto.\nSe precisar sintetizar trechos longos, produza um resumo fiel e objetivo, em português claro.\n\nHoje é: ${new Date().toLocaleDateString('pt-BR')}\nAno: ${new Date().getFullYear()}\n\nContexto:\n${contextText}`
                    },
        ...
        try {
            ...
            const ejson = JSON.parse(ev.choices[0].message.content);
            if (ejson.needs_more_info === true || (ejson.confidence || 0) < 0.6) {
                ...
                const altMerged = {};
                altMatches.forEach(d => { const k = d.id; altMerged[k] = altMerged[k] ? altMerged[k] : d; });
                const altDocs = Object.values(altMerged).slice(0, 15);
                const altParentsMap = {};
                for (const d of altDocs) {
                    ...
                    const ordered = (chunks || []).sort((a, b) => (a.metadata?.chunk_index || 0) - (b.metadata?.chunk_index || 0));
                    const full = ordered.map(c => c.content).join('\n');
                    const key = `${sid}::${title}::${type || ''}`;
                    if (!altParentsMap[key]) altParentsMap[key] = { content: full, metadata: d.metadata };
                }
                const extraParents = Object.values(altParentsMap);
                for (const p of extraParents) {
                    const t = `[Transcrição: ${p.metadata.session_title} | Bloco: ${p.metadata.title}]\n${p.content}\n\n`;
                    if (totalChars + t.length < maxContextChars) {
                        contextText += t;
                        totalChars += t.length;
                        sources.push(p);
                    }
